...
export async function registerConversation(
  ...
  const remember = await conversation.external(() =>
    getConfig("rememberUserData"),
  );
  ...
  while (true) {
    ...
    const buffer = await conversation.external(() =>
      downloadFileBuffer(ctx, screenshotFileId),
    );
...
async function collectUserData(
  ...
  while (true) {
    ...
    if (parts.length < 2) {
      await nameCtx.reply(
        "Введіть ім'я та прізвище через пробіл (наприклад: Марія Іваненко)",
      );
      continue;
    }
