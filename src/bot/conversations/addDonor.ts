...
export async function addDonorConversation(
  ...
  const activeEventId = await conversation.external(() =>
    getConfig("activeEventId"),
  );
  if (!activeEventId) {
  ...
  // Phone
  await ctx.reply(
    "Введіть номер телефону учасника (наприклад: +380501234567):",
  );
  ...
  const canAdd = await conversation.external(() =>
    hasAvailableTickets(activeEventId),
  );
  if (!canAdd) {
  ...
  const tickets = await conversation.external(() =>
    generateAvailableTickets(activeEventId),
  );
  await ctx.reply(
  ...
  logger.info(
    { donationId: result.donation.id, firstName, lastName },
