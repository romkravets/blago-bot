...
export function adminHandlers(bot: Bot<MyContext>): void {
  ...
  bot.callbackQuery(/^approve:(.+)$/, requireAdmin, async (ctx) => {
    ...
    if (noTickets) {
      ...
      await ctx.editMessageCaption({
        caption: "⚠️ Підтвердити неможливо — всі номерки розібрані",
      });
      return;
    }
    ...
    try {
      ...
      logger.error(
        { err, userId: donation.user.telegramId },
        "Failed to send ticket options to user",
      );
    }
  ...
  bot.command("stats", requireAdmin, async (ctx) => {
    ...
    const approved = await prisma.donation.count({
      where: { eventId: activeEventId, status: "APPROVED" },
    });
    const rejected = await prisma.donation.count({
      where: { eventId: activeEventId, status: "REJECTED" },
    });
  ...
  bot.command("pending", requireAdmin, async (ctx) => {
    ...
    await ctx.reply(
      `⏳ Очікують підтвердження (${donations.length}):\n\n${lines.join("\n\n")}`,
    );
  });

  // ── /export ───────────────────────────────────────────────────────
  bot.command("export", requireAdmin, async (ctx) => {
    await ctx.reply("Оберіть формат експорту:", {
  ...
  bot.callbackQuery("export:csv", requireAdmin, async (ctx) => {
    ...
    data.forEach((d) =>
      csv.push(
        `${d.chosenTicket},${d.user.firstName},${d.user.lastName},${d.user.phone}`,
      ),
    );
  ...
  bot.callbackQuery("export:names", requireAdmin, async (ctx) => {
    ...
    const data = await getExportData();
    const text = data
      .map((d) => `${d.user.firstName} ${d.user.lastName}`)
      .join("\n");
  ...
  bot.command("new_raffle", requireSuperAdmin, async (ctx) => {
    ...
    logger.info(
      { eventId: newEvent.id, title, maxTickets },
      "New raffle created",
    );
  });
