...
Учасники роблять донат на благодійний рахунок, надсилають скріншот підтвердження і отримують номерок. Переможця обирає організатор вручну (wheelofnames.com).

📋 Як взяти участь:
1. Зроби переказ на вказаний благодійний рахунок
...
export function userHandlers(bot: Bot<MyContext>): void {
  ...
  bot.hears("🎟 Взяти участь", rateLimit, async (ctx) => {
    ...
    if (existing) {
      await ctx.reply(
        'Ви вже зареєстровані в цьому розіграші. Перевірте статус — натисніть "📋 Мій номерок".',
      );
      return;
    }
  ...
  async function handleMyTicket(ctx: MyContext): Promise<void> {
    ...
    if (donation.status === "PENDING" && !donation.chosenTicket) {
      await ctx.reply(
        "⏳ Ваш запит на розгляді. Очікуйте підтвердження адміна.",
      );
      return;
    }
    ...
    if (donation.status === "REJECTED") {
      await ctx.reply(
        "❌ Ваш запит було відхилено. Зверніться до організатора @KravetsNaastia.",
      );
      return;
    }
  }
  ...
  bot.callbackQuery(/^ticket:(\d+):(.+)$/, async (ctx) => {
    ...
    logger.info(
      { donationId, ticketNumber, userId: ctx.from.id },
      "Ticket claimed",
    );
  });
