...
export function userHandlers(bot: Bot<MyContext>): void {
  bot.command("start", async (ctx) => {
    ...
    if (existing) {
      await ctx.reply(
        "Ви вже зареєстровані в цьому розіграші. Перевірте статус — /my_ticket",
      );
      return;
    }
    ...
    await ctx.reply(
      "🎁 Вітаємо в благодійному розіграші!\n\nДля участі вам потрібно:\n1. Вказати ім'я та прізвище\n2. Поділитися номером телефону\n3. Надіслати скріншот оплати",
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "🎟 Зареєструватись", callback_data: "start_register" }],
          ],
        },
      },
    );
  ...
  bot.command("my_ticket", async (ctx) => {
    ...
    if (donation.status === "REJECTED") {
      await ctx.reply(
        "❌ Ваш запит було відхилено. Зверніться до організатора.",
      );
      return;
    }
  });
